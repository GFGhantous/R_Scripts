...
m1<-gamlss(y~1,tau.start=4,tau.fix=TRUE,data=dados1,family=BCT(mu.link=log))
...
mu=exp(preditor)
sigma=exp(sigma)
sum(dBCT(Y,mu=mu,sigma=sigma,nu=nu,tau=4,log=TRUE))+
...
as.numeric(coef(P.GH)[3]),4,1/exp(as.numeric(coef(P.GH)[4])))
...
verdadeiroparametro<- c(5,0.5,0.5,sqrt(0.5))
