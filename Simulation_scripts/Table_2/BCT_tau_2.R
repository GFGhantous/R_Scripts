...
y=rBCCG(1500,mu=mu,sigma=0.5,nu=0.5)
dados1=data.frame(ind,rec,y)


m1<-gamlss(y~1,tau.start=2, tau.fix=TRUE,data=dados1,family=BCT(mu.link=log))
...
mu=exp(preditor)
sigma=exp(sigma)
sum(dBCT(Y,mu=mu,sigma=sigma,nu=nu,tau=2,log=TRUE))+
...
as.numeric(coef(P.GH)[3]),2,1/exp(as.numeric(coef(P.GH)[4])))
...
verdadeiroperc<-c(46.20388,59.80620,92.16034,148.3078,238.50553,366.33252,472.66929)
